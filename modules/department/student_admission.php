...
if ($_POST) {
    try {
        if (isset($_POST['action'])) {
            ...
            if (isset($_POST['id'])) {
                error_log("ID received: " . $_POST['id']);
            }
            ...
            if ($_POST['action'] === 'add') {
                ...
                error_log("Delete action triggered for ID: " . $_POST['id']);
                
                // First, get the file path to delete the file
                $query = "SELECT documents_path FROM student_admission WHERE id = :id";
                $stmt = $db->prepare($query);
                ...
                if ($stmt->execute()) {
                    ...
                    if ($rowsDeleted > 0) {
                        ...
                        if ($record && $record['documents_path'] && file_exists($record['documents_path'])) {
                            unlink($record['documents_path']);
                        }
                        $message = 'Student admission record deleted successfully! (ID: ' . $_POST['id'] . ')';
                        $messageType = 'success';
                    } else {
                        $message = 'No record found with ID: ' . $_POST['id'] . '. It may have already been deleted.';
                        $messageType = 'error';
                    }
                } else {
                    $errorInfo = $stmt->errorInfo();
                    error_log("Delete failed: " . print_r($errorInfo, true));
                    $message = 'Error deleting student admission record: ' . $errorInfo[2];
                    $messageType = 'error';
                }
            }
        }
...
<body>
    ...
    <script>
        // Auto-save form data (only for add form, not delete forms)
